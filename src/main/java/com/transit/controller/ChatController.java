...
public class ChatController {
    ...
    public Mono<ResponseEntity<?>> chatCompletions(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authorization,
        ...
        return transitService.chatCompletions(authorization, request, clientIp)
    ...
    public Mono<Map<String, Object>> models(
        ...
        return Mono.fromCallable(() -> Map.of(
                "object", "list",
                "data", transitService.availableModels(authorization, clientIp(httpRequest)).stream()
                        .map(model -> Map.of(
                                "id", model,
                                "object", "model",
                                "created", Instant.now().getEpochSecond(),
                                "owned_by", "api-transit"
                        ))
                        .toList()
        ));
    }
