...
class ListPageAdviceTests {
 ...
 @Test void defaultsToTenRowsWhenSizeIsOmitted()throws Exception{mvc.perform(get("/rows").param("listPage","true")).andExpect(status().isOk()).andExpect(jsonPath("$.size").value(10)).andExpect(jsonPath("$.items.length()").value(10)).andExpect(jsonPath("$.items[9].id").value(9));}
