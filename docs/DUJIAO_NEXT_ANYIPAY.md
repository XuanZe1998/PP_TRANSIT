...
```text
本项目 Plus 订单 -> 安易付 V2 -> /webhooks/anyipay -> plus_orders
...
`notify_url` 绝对不能填成本项目的 `/webhooks/anyipay`。本项目的回调只允许 `param=plus-order:<id>`
的 Plus 订单，Dujiao-Next 回调会被拒绝。
...
anyipay:
  ...
  return-url: https://modelhub.example.com/plus
```

两套系统可以使用同一安易付商户凭据，但必须保持：

1. Plus 订单号使用 `PLUS...` 前缀；
