<template>
  <section class="commerce-admin">
    <div class="panel-title"><h3>商品与履约配置</h3><el-select v-model="selectedId" placeholder="选择当前页服务" style="width:320px" @change="selectService"><el-option v-for="item in services" :key="item.id" :label="item.name" :value="item.id" /></el-select></div>
    <ListPagination v-model:page="servicePage" v-model:size="serviceSize" :total="serviceTotal" :allow-all="false" @change="loadServices" />
