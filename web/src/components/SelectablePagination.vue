<template><ListPagination :total="total" :page="currentPage" :size="pageSize===200?DEFAULT_PAGE_SIZE:pageSize" :all="pageSize===200" @update:page="setPage" @update:size="setSize" @update:all="setAll" @change="emit('change',nextPage,nextSize)"/></template>
<script setup lang="ts">
import {onMounted} from 'vue'
import ListPagination from './ListPagination.vue'
import {DEFAULT_PAGE_SIZE} from '../utils/listPage'
const props=withDefaults(defineProps<{total?:number;currentPage?:number;pageSize?:number;listId:string}>(),{total:0,currentPage:1,pageSize:DEFAULT_PAGE_SIZE})
...
function setAll(value:boolean){if(value)setSize(200);else if(props.pageSize===200)setSize(DEFAULT_PAGE_SIZE)}
