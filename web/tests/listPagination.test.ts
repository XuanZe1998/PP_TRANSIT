...
describe('business list pagination',()=>{
 ...
 it('uses twenty rows as the shared default page size',()=>{const listPage=readFileSync(resolve('src/utils/listPage.ts'),'utf8');const selectable=readFileSync(resolve('src/components/SelectablePagination.vue'),'utf8');expect(listPage).toContain('? saved.size! : 20');expect(selectable).toContain('pageSize:20');expect(selectable).toContain('pageSize===200?20:pageSize');expect(selectable).toContain('setSize(20)')})
