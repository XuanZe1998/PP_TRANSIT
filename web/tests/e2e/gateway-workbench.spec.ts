...
test('gateway uses site/group hierarchy, red failures, unified jobs and selectable pages',async({page})=>{
 ...
 await page.route('**/api/**',async route=>{const url=new URL(route.request().url());calls.push(url.pathname+'?'+url.searchParams.toString());
  ...
  if(url.pathname.endsWith('/gateway/groups')){let rows=groups;if(url.searchParams.get('siteId'))rows=rows.filter(x=>x.site_id===Number(url.searchParams.get('siteId')));const size=Number(url.searchParams.get('size')||10),current=Number(url.searchParams.get('page')||1);return route.fulfill({json:{items:url.searchParams.get('all')==='true'?rows:rows.slice((current-1)*size,current*size),total:rows.length,page:current,size}})}
  if(url.pathname.endsWith('/gateway/sync'))return route.fulfill({json:{jobs:[{groupId:1,jobId:'job-test'}]}})
  if(url.pathname.endsWith('/gateway/jobs'))return route.fulfill({json:{items:[{id:'job-test',channel_id:1,site_name:'ahh',group_name:'Claude-Max',status:'ERROR',message:'上游凭据无效或已过期',suggestion:'更新该分组 Key 后重试',http_status:401,phase:'MODELS'}],total:1,page:1,size:10}})
  return route.fulfill({json:{}})
 })
 await page.goto('/admin/model-gateway')
 const workbench=page.locator('.gateway-workbench')
 await expect(workbench.getByRole('heading',{name:'模型网关',level:1})).toHaveCount(1)
 await expect(page.locator('.admin-header')).not.toContainText('模型网关')
 await expect.poll(()=>calls.some(x=>x.includes('/admin/api/gateway/groups')&&x.includes('size=10'))).toBeTruthy()
 const headingBox=await workbench.locator('.admin-page-toolbar__heading').boundingBox()
 const controlsBox=await workbench.locator('.admin-page-toolbar__controls').boundingBox()
 expect(headingBox).not.toBeNull()
 expect(controlsBox).not.toBeNull()
 expect(controlsBox!.y).toBeGreaterThan(headingBox!.y+headingBox!.height)
 ...
 await workbench.locator('.admin-page-toolbar__filters .el-select').first().click()
 ...
 await expect(workbench.locator('.admin-page-toolbar__controls')).toHaveCSS('flex-direction','column')
 await expect(workbench.locator('.admin-page-toolbar__filters')).toHaveCSS('flex-direction','column')
